...
const updateDepthChart = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO depth_charts (team_id, position, depth_order, player_id)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (team_id, position, depth_order)
       DO UPDATE SET player_id = $4
       RETURNING *`,
      [teamId, position, depthOrder, playerId]
    );
